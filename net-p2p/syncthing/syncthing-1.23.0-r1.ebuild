...
inherit desktop go systemd xdg-utils
...
src_compile() {
	echo /home/ryan/Programs/go/bin/go run build.go -version "v${PV}" -no-upgrade -build-out=bin/ \
		${GOARCH:+-goarch="${GOARCH}"} -gocmd /home/ryan/Programs/go/bin/go \
		build $(usex tools "all" "")
	GOARCH= /home/ryan/Programs/go/bin/go run build.go -version "v${PV}" -no-upgrade -build-out=bin/ \
		${GOARCH:+-goarch="${GOARCH}"} -gocmd /home/ryan/Programs/go/bin/go \
