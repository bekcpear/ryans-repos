...
default_dir="@EROOT@/var/lib/tailscale"
...
docp() {
	...
	if [[ -f $_default_state_c ]]; then
		if [[ -f $_default_state && $(wc -c $_default_state | cut -d' ' -f1) -lt 10 ]] || \
			...
			warn "Existing a tailscale state in the path '@EROOT@/var/lib/tailscale/tailscaled.d',"
			...
			warn "But there is also a configured state file with path '@EROOT@/var/lib/tailscale/tailscaled.state',"
