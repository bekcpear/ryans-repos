...
pkg_postinst() {
	...
	if [[ -z ${REPLACING_VERSIONS} ]] ; then
		...
		if (( $major == 0 && $minor < 23 )); then
			...
			if (( $major > 0 )) || (( $major == 0 && $minor >= 23 )); then
				ewarn ">=headscale-0.23.0-alpha1 has done the code reorganisation,"
				ewarn "please review the issue: https://github.com/juanfont/headscale/issues/1473"
				ewarn "**BACKUP** your database before restarting!!"
			fi
		fi
	fi
}
